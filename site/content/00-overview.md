...
建议按顺序读。第 1 章把近 200 个 crate 归了类，免得你在 `core/src` 深处迷路；第 2 章讲三个进程怎么协作。第 3、4 章是重头，专门啃 `codex-core` 的公开 API 和主循环，读下来基本就摸透了项目是怎么驱动的；往后依次是工具系统、沙箱与安全、TUI、协议、配置，最后到执行与运行时、扩展生态、模型与上下文、工程化。
