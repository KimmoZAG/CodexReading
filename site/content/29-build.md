...
可重现构建是第一动力：bazel 用 hermetic action 与内容寻址缓存，把 C++/Swift/Rust 跨语言依赖（如 `v8`、OpenSSL、alsa）锁在同一类沙箱里。`rules_rs`（`reference/codex/MODULE.bazel:148`）提供 Rust 规则，`toolchains.toolchain(edition = "2024", version = "1.95.0")`（`reference/codex/MODULE.bazel:245`）统一工具链版本。跨语言（Rust 通过 bindgen 生成 C 绑定、调 V8）、多平台产物（10 种 target triple 列在 `crate.from_cargo` 的 `platform_triples` 中），以及 CI 的远程缓存命中率——这些都是 cargo 难以低成本提供的。cargo 关心"能不能编译"，bazel 关心"在任何机器、任何时间编译出逐字节一致的产物"。
