...
这本 guide 一共 16 章：开头的引子、第 1 章到第 13 章这十三章正文，以及结尾的术语表与架构全景、阅读路线两页参考。把引子、第 1 章到第 9 章（仓库地图一路读到配置系统）走完，Codex 的驱动轴和主干流程就已经装进你脑子里了：三进程模型、core 的 `submit/next_event` 驱动轴、采样—执行主循环、工具与沙箱、TUI 作为事件视图、协议把大脑变成服务、配置把一切串起来。后面第 10–13 章是围绕它展开的执行链路、扩展生态、模型与上下文、工程化，按你的目标挑读就行。
